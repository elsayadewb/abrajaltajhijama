...
<script>
    $(document).ready(function(){
        ...
        $(".message.success_settings").css({right: '274px'},3500);
        $(".message.success_settings").stop().delay("2720").animate({right: '-331px'},400);
         // $(".swal-overlay--show-modal").stop().delay("10020").css("display","none",10500);
        $(".swal-overlay--show-modal").delay("3000").fadeOut();
...
@if( setting()->alert_success == 'modal' OR setting()->alert_success == '' )
...
@if(session()->has('successhome'))
    ...
    <script>
        swal({
            title: "Good job!",
            text: "{{ session()->get('successhome') }}",
            icon: "success",
            button: "{{trans('admin.yes')}}",
        });
...
@endif
@if(session()->has('success'))
    <div class="alert btn-success  alert_success_home message    "> <img class="w_22px_h_22px "src="{{url('default')}}/like7.png" > {{ session()->get('success') }} <img class="w_25px_h_25px m_b_6px"src="{{url('default')}}/clap.png" ></div>
@endif
